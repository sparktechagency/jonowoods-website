...
const SubscriptionBeforeLogin = () => {
  ...
  const PackageCard = ({ pkg, isPopular = false }) => (
    <Card
      className={`relative overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 border-2 ${
        isPopular
          ? "border-[#ca3939] ring-4 ring-[#ca3939]/10 shadow-xl"
          : "border-slate-200 hover:border-[#ca3939]/30"
      }`}
    >
      {isPopular && (
        <div className="absolute top-0 left-0 right-0 bg-gradient-to-r from-[#ca3939] to-[#7a141a] text-white text-center py-3 font-semibold text-sm">
          🔥 Most Popular Choice
        </div>
      )}

      <CardHeader className={`space-y-4 ${isPopular ? "pt-16" : "pt-8"}`}>
        ...
        <CardTitle className="text-3xl font-bold text-slate-900 group-hover:text-[#ca3939] transition-colors">
      ...
      <CardContent className="space-y-8">
        ...
        <Button
          ...
          className="w-full bg-[#ca3939] hover:bg-[#7a141a] text-white font-bold py-4 text-lg transition-all duration-300 hover:scale-105 active:scale-95 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
        >
        ...
        >
          {webPackages.map((pkg, index) => (
            <PackageCard
              key={pkg._id}
              pkg={pkg}
              isPopular={index === 0 || pkg.discount > 20}
            />
          ))}
        </div>
